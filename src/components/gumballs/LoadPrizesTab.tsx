...
export const LoadPrizesTab = ({gumballId}: {gumballId: string}) => {
  ...
  const totalPrizesAdded = gumball?.prizesAdded || 0;
  const maxROI = useMemo(()=>{
    const roi = ((gumball?.totalTickets * parseFloat(gumball?.ticketPrice)/(10**(VerifiedTokens.find((token: typeof VerifiedTokens[0]) => token.address === gumball?.ticketMint)?.decimals || 0)) - parseFloat(gumball?.totalPrizeValue)) / parseFloat(gumball?.totalPrizeValue)) * 100;
    if(isNaN(roi) || roi === Infinity) return 0;
    return Math.max(roi, 0).toFixed(2); 
  }, [gumball?.totalTickets, gumball?.ticketPrice, gumball?.ticketMint, gumball?.totalPrizeValue]);
  console.log(maxROI);
         ...
         <div className="flex items-center gap-5 border border-solid border-primary-color rounded-[10px] bg-primary-color/5 py-4 px-5">
          ...
          <div className="w-full grid md:grid-cols-4 grid-cols-2 my-10 bg-gray-1300 border border-gray-1100 p-6 gap-20 rounded-[10px]">
            <div className="">
                ...
                <h4 className='text-2xl font-bold font-inter text-black-1000'>{gumball?.prizes.length}/{gumball?.maxPrizes}</h4>
            </div>

               <div className="">
                <h3 className='text-base text-black-1000 font-medium font-inter mb-[22px]'>Total Prize Value</h3>
                <h4 className='text-2xl font-bold font-inter text-black-1000'>{(parseFloat(gumball?.totalPrizeValue)/(10**9)).toFixed(2)} SOL</h4>
            </div>

               <div className="">
                <h3 className='text-base text-black-1000 font-medium font-inter mb-[22px]'>Max Proceeds</h3>
                <h4 className='text-2xl font-bold font-inter text-black-1000'>{gumball?.totalTickets * parseFloat(gumball?.ticketPrice)/(10**(VerifiedTokens.find((token: typeof VerifiedTokens[0]) => token.address === gumball?.ticketMint)?.decimals || 0))} SOL</h4>
            </div>
